export const Bio = {
  name: "Rahul Kumar",
  roles: ["Front-End Developer", "UI/UX Designer", "Programmer"],
  description:
    "I am a motivated and versatile individual, always eager to take on new challenges. With a passion for learning I am dedicated to delivering high-quality results. With a positive attitude and a growth mindset, I am ready to make a meaningful contribution and achieve great things.",
  github: "https://github.com/krahul09",
  resume:
    "https://drive.google.com/file/d/1RanefOxIofWKvd3BxWrPEKl8iXZAONcj/view?usp=drive_link",
  linkedin: "https://www.linkedin.com/in/rahul-kumar-068726199/",
  twitter: "https://twitter.com/raulk25__",
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      {
        name: "TypeScript",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Redux",
        image:
          "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
      },
      {
        name: "Tailwind CSS",
        image:
          "https://picperf.io/https://laravelnews.s3.amazonaws.com/images/tailwindcss-1633184775.jpg",
      },
      {
        name: "Material UI",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
      {
        name: "Angular",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Next Js",
        image:
          "data:image/png;base64,",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Graph Ql",
        image: "https://www.vectorlogo.zone/logos/graphql/graphql-icon.svg",
      },
      {
        name: "MongoDB",
        image:
          "https://i.pcmag.com/imagery/reviews/02Q6yxveinggAu3PomearaV-7.fit_scale.size_760x427.v1569481734.jpg",
      },
      {
        name: "Firebase",
        image: "https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg",
      },
    ],
  },

  {
    title: "Others",
    skills: [
      {
        name: "Git",
        image:
          "https://res.cloudinary.com/practicaldev/image/fetch/s---Xg28U2u--/c_imagga_scale,f_auto,fl_progressive,h_420,q_auto,w_1000/https://dev-to-uploads.s3.amazonaws.com/i/q7uy4yxekcljpr70p2xk.png",
      },
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
      {
        name: "Figma",
        // image:
        //   "https://cdn.dribbble.com/users/2653319/screenshots/6813714/figma_logo_animation.gif",
        image:
          "https://i.pinimg.com/564x/18/f1/72/18f1727873924ba58fde1f739d11b77b.jpg",
      },
      {
        name: "Hoppscotch",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTHlIt1HrnV1bmZaewJbtha9XzhLpR0OLTr_A&usqp=CAU",
      },
      {
        name: "Postman",
        image:
          "https://miro.medium.com/v2/resize:fit:1400/format:webp/1*QOx_tPV5wJnhTzAGhfIiLA.png",
      },
      {
        name: "Adobe XD",
        image:
          "https://camo.githubusercontent.com/c205ecbe12500177d102169d97bc1c17c545155fdf5ec78c08d54ac53e5b38c1/68747470733a2f2f63646e2e776f726c64766563746f726c6f676f2e636f6d2f6c6f676f732f61646f62652d78642e737667",
      },
    ],
  },
];

export const experiences = [
  {
    id: 0,
    img: "https://media.licdn.com/dms/image/D560BAQFKKOxcY9V8eQ/company-logo_200_200/0/1681700877053/talentmonksgp_logo?e=1710374400&v=beta&t=jWjtsm_Ou8db2XE9u409IkR9AEL8TRTob53lOHd6etY",
    role: "Front-End Developer",
    company: "Talent Monk",
    date: "Jul 2023 - Dec 2023",
    desc: "• Utilized my expertise as a Frontend Developer with proficiency in technologies such as ReactJS , Redux , tailwind CSS and others to implement interactive surveys, user‑friendly forms, and post modules. This contributed significantly to enhancing user engage‑ment and streamlining data collection processes.",
    skills: [
      "HTML",
      "CSS",
      "JavaScript",
      "ReactJs",
      "Redux",
      "Tailwind CSS",
      "React Query",
    ],
  },
  {
    id: 1,
    img: "https://media.licdn.com/dms/image/D560BAQEfN3cBTUKHmg/company-logo_200_200/0/1687544906714/mlearningintelligence_logo?e=1710374400&v=beta&t=UEXLGYkv71KNGsM2-rcjIRouHczY8RG6YkaS8K050cA",
    role: "Front-End Developer (Freelance)",
    company: "Machine Learning Intelligence",
    date: "Dec 2022 - May 2023",
    desc: "• Developed and designed user‑friendly interfaces using Angular, TypeScript ensuring seamless user interactions and efficient data visualization. Integrated front end with GraphQL, handling queries and mutations to efficiently fetch and update real‑time market data.",
    skills: ["TypeScript", "Angular", "Bootstrap", "GraphQL"],
  },
  {
    id: 2,
    img: "https://media.licdn.com/dms/image/C4E0BAQHmdC9IposUjw/company-logo_200_200/0/1630643654750?e=1710374400&v=beta&t=C2wIgNXF1NFHZn63sGoD99qJ5hGZzfV-mE7N69bAN9c",
    role: "Front-End Developer (Intern)",
    company: "Helpy Moto",
    date: "Mar 2022 - Dec 2022",
    desc: "• Collaborated with the team to build the initial stage of the application, Utilized technologies such as ReactJS, Figma , Material UI , Redux",
    skills: [
      "HTML",
      "CSS",
      "JavaScript",
      "ReactJS",
      "Redux",
      "Figma",
      "Material UI",
    ],
  },
  {
    id: 3,
    img: "https://media.licdn.com/dms/image/C510BAQHS-o4ahqf3Vw/company-logo_200_200/0/1630613500649/minzor_logo?e=1710374400&v=beta&t=DHWnqmRigSgrXIHfbg2shHdhcQ8sbzNqzg73AyFqAgU",
    role: "Web Developer",
    company: "Minzor Private Limited",
    date: "Jan 2021 - Jun 2021",
    desc: "• Worked on impactful projects like Fit Bharat Family and Thecodingbus. Worked with WordPress technology. • Fit Bharat Family campaign impacted and reached 10-20k people nationwide. • Thecodingbus assists users in finding coding resources, learning paths, and has been used by 1-2L people and growing..",
    skills: ["Word Press"],
  },
  {
    id: 4,
    img: "https://media.licdn.com/dms/image/C560BAQHTvZwCx4p2Qg/company-logo_200_200/0/1630640869849/amazon_logo?e=1710374400&v=beta&t=trMtkPXcjySzk3rBJ6b1CdjkpLv2TWyViO-PjRHo9Lk",
    role: "Quality Analyst (Non-Tech)",
    company: "Amazon",
    date: "Dec 2021 - June 2023",
  },
];

export const education = [
  {
    id: 0,
    img: "https://upload.wikimedia.org/wikipedia/en/a/ae/J.C._Bose_University_of_Science_and_Technology%2C_YMCA_logo.png",
    school:
      "J.C. Bose University of Science and Technology, YMCA (Formerly YMCA UST)",
    date: "Jun 2021",
    degree:
      "Bachelor of Technology - BTech, Electronics and Communiaction Engineering",
  },
  {
    id: 1,
    img: "https://1.bp.blogspot.com/-Y2Mqj01Rx7o/WIQaMix1MjI/AAAAAAAAFAY/HnvQGqbnbjMBeYINyo_c3fClBvZPPvv9ACLcB/s320/kvs-logo.png",
    school: "Kendriya Vidyalaya Janak Puri",
    date: "Apr 2016",
    degree: " XII ",
  },
  {
    id: 2,
    img: "https://1.bp.blogspot.com/-Y2Mqj01Rx7o/WIQaMix1MjI/AAAAAAAAFAY/HnvQGqbnbjMBeYINyo_c3fClBvZPPvv9ACLcB/s320/kvs-logo.png",
    school: "Kendriya Vidyalaya - No.1 AFS, Suratgarh",
    date: "Mar 2014",
    degree: " X ",
  },
];

export const projects = [
  {
    id: 0,
    title: "Fitness King",
    description:
      "• Comprehensive exercise search based on preferences and fitness goals. •User-friendly interface with intuitive navigation, detailed instructions, and demonstration videos. • Accessible to users of all levels, providing a seamless and informative fitness experience",
    image:
      "https://user-images.githubusercontent.com/81851334/289255184-0edccce4-f016-4944-86e8-79629602aa9e.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..UloJ3O4Sv33l1i6ys2VCJrO3oyP_QW_4wHNsR4SkO4I",
    tags: [
      "HTML",
      "CSS",
      "JavaScript",
      "ReactJs",
      "Material UI ,",
      "Rapid API",
      "Redux",
    ],
    github: "https://github.com/krahul09/gym-app",
    webapp: "https://gym-app-tkjx.vercel.app/",
  },
  {
    id: 1,
    title: "Movie Stats App",
    description:
      "• The ultimate companion for movies and TV shows, featuring descriptions, ratings, and recommendations. • Dive into detailed summaries, cast information, and genre tags for thousands of films and shows.",
    image:
      "https://user-images.githubusercontent.com/81851334/289255287-994af772-d8ee-428b-976e-ea528199285c.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..2n_OjNBa9XwzelgWG8NCHhMNcwyDT4pQ5HrJ7rmql-s",
    tags: [
      "HTML",
      "CSS",
      "JavaScript",
      "ReactJs",
      "Material UI ,",
      "Rapid API",
      "Redux",
    ],
    github: "https://github.com/krahul09/movie-stats-app",
    webapp: "https://movie-stats-app.vercel.app/",
  },
  {
    id: 2,
    title: "Music Player",
    description:
      "• Web-based music player with pre-set music selection for easy playback.",
    image:
      "https://user-images.githubusercontent.com/81851334/289255265-1aa1e8b4-cf5f-4685-b1f9-52bf6eb78009.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..eMJSyNPTDcEG_Z54vaeb7MtRM7uVYajXv9rnCFzkzCA",
    tags: [
      "HTML",
      "CSS",
      "JavaScript",
      "ReactJs",
      "Material UI ,",
      "Rapid API",
      "Redux",
    ],
    github: "https://github.com/krahul09/Music-Player",
    webapp: "https://music-player-nine-psi.vercel.app/",
  },
];
